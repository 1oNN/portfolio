...
export function VoiceFlowArchitecture({ accent, className }: Props) {
  ...
  return (
    ...
    >
      ...
      <PvBox accent={accent} x={20} y={120} w={140} h={60} title="Retell API" sub="calls · audio · metadata" delay={0} />
      <PvBox accent={accent} x={220} y={120} w={160} h={60} title="FastAPI Job Manager" sub="async export jobs" highlight delay={0.15} />
      <PvBox accent={accent} x={460} y={120} w={170} h={60} title="Whisper large-v3" sub="local · GPU when available" delay={0.3} />
      <PvBox accent={accent} x={710} y={120} w={160} h={60} title="JSON Export" sub="selected columns + transcripts" delay={0.45} />

      {/* UI above the job manager, fed by SSE */}
      <PvBox accent={accent} x={220} y={210} w={160} h={50} title="Next.js UI" sub="columns · dates · progress" delay={0.6} />
      <PvArrow accent={accent} markerId="vflow-arrow" x1={300} y1={210} x2={300} y2={180} dashed delay={0.8} />
      <text x={312} y={200} fontFamily={MONO} fontSize="9" fill={muted} className="pv-fade" style={{ animationDelay: "0.9s" }}>
        SSE logs + progress
      </text>

      {/* Main flow */}
      <PvArrow accent={accent} markerId="vflow-arrow" x1={160} y1={150} x2={220} y2={150} delay={0.1} />
      <PvArrow accent={accent} markerId="vflow-arrow" x1={380} y1={150} x2={460} y2={150} delay={0.25} />
      <PvArrow accent={accent} markerId="vflow-arrow" x1={630} y1={150} x2={710} y2={150} delay={0.4} />

      <PvFlowDot accent={accent} path="M 160 150 L 220 150" dur={2.4} delay={0} />
      <PvFlowDot accent={accent} path="M 380 150 L 460 150" dur={2.4} delay={0.8} />
      <PvFlowDot accent={accent} path="M 630 150 L 710 150" dur={2.4} delay={1.6} />
