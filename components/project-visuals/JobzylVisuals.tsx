  ...
  return (
    ...
    >
      ...
      {jobs.map((j, i) => {
        ...
        return (
          <g key={j.co}>
            <rect x="220" y={y} width="520" height="58" rx="6" ry="6" fill={surface} stroke={border} strokeWidth="1" />
            {/* Logo placeholder */}
...
export function JobzylArchitecture({ accent, className }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Aggregator */}
      <Box x={210} y={200} w={140} h={56} title="FastAPI" sub="aggregator · App Runner" />

      {/* Supabase */}
      <Box x={400} y={170} w={150} h={56} title="Supabase" sub="11 RLS-locked tables" highlight />

      {/* Scheduler (cache warming) */}
      <Box x={400} y={260} w={150} h={44} title="Scheduler" sub="every 6h re-scrape" />

      {/* SSE */}
      <Box x={600} y={170} w={130} h={56} title="SSE Stream" sub="live progress" />

      {/* Client */}
      <Box x={770} y={140} w={110} h={50} title="Next.js" sub="dashboard" />
      <Box x={770} y={230} w={110} h={50} title="ATS Score" sub="client-side" />
      ...
      {/* Aggregator → Supabase */}
      <line x1={350} y1={222} x2={400} y2={198} stroke={accent} strokeWidth="1.25" opacity="0.7" markerEnd="url(#jobzyl-arrow)" />
      {/* Aggregator → Scheduler */}
      <line x1={350} y1={234} x2={400} y2={282} stroke={accent} strokeWidth="1.25" opacity="0.55" markerEnd="url(#jobzyl-arrow)" strokeDasharray="3 3" />
      {/* Scheduler → Supabase (cache warming feedback) */}
      <line x1={475} y1={260} x2={475} y2={226} stroke={accent} strokeWidth="1" opacity="0.45" strokeDasharray="3 3" />

      {/* Supabase → SSE */}
      <line x1={550} y1={198} x2={600} y2={198} stroke={accent} strokeWidth="1.25" opacity="0.7" markerEnd="url(#jobzyl-arrow)" />
      {/* SSE → Next.js */}
      <line x1={730} y1={188} x2={770} y2={165} stroke={accent} strokeWidth="1.25" opacity="0.7" markerEnd="url(#jobzyl-arrow)" />
      {/* Next.js → ATS (client-only) */}
      <line x1={825} y1={190} x2={825} y2={230} stroke={accent} strokeWidth="1.25" opacity="0.55" markerEnd="url(#jobzyl-arrow)" strokeDasharray="3 3" />
