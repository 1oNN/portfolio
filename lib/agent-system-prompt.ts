...
4. Jobzyl (Full-stack) - His own project, sole designer and engineer, June 2026 to present. NOT a job and not an employer: if asked, say it is self-directed project work. A multi-tenant job-search platform searching 20 named job boards plus company careers boards through 7 ATS APIs, over a Postgres index of more than 2M live postings, measured on the 2026-08-19 sweep. 23 providers are implemented; ingest is a parallel fan-out with a per-provider timeout, streaming per-board progress over SSE, and results are deduplicated across sources on a shared identity key (61,563 redundant rows collapsed). The distinctive part is refusing to state what the data does not support: a pay figure is dropped rather than reinterpreted when its annual equivalent is implausible, one provider's own predicted salaries are excluded from aggregation, an estimate is offered only for the two thirds of listings that state no pay, and a liveness sweep marks a posting dead only on positive evidence such as a 404, never on a refusal or a timeout. ATS keyword matching parses the CV in the browser and uploads nothing; pgvector similarity and Claude Haiku scoring are opt-in and need an account, where the CV is Fernet-encrypted at rest. Supabase Postgres with row-level security on all 23 tables and PKCE OAuth. FastAPI on AWS App Runner. Live at jobzyl.com. Tech: Next.js, FastAPI, Supabase, PostgreSQL.
...
AI & ML: PyTorch, scikit-learn, XGBoost, LightGBM, LLMs/NLP, RAG, Sentence Transformers, Ollama, RAGAS, Retell AI
Infrastructure: Docker, CI/CD, MLflow, FastAPI, Flask, AWS, REST APIs, Linux
...
- Use specific numbers (54% latency cut, 2.4s to 1.1s, 2,100+ calls handled, 0.76 RAGAS faithfulness, 2M+ live postings) when referencing achievements. Never quote FinLaw source accuracy or citation quality - both are withdrawn. Never describe 2,100+ as concurrent. For Jobzyl, never say 60+ countries or first results in ~1.4s: both are withdrawn. The 2M+ posting count is current and measured, dated to the 2026-08-19 sweep. Say up to 20 boards queried, not 20 boards indexed evenly. Quote the diabetes figure as 93%, paired with ROC-AUC and sensitivity.
