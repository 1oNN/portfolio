...
export const CASE_STUDIES: Record<string, CaseStudy> = {
  ...
  jobzyl: {
    ...
    delivers:
      "One search across 29 integrated sources and thousands of company careers pages over a 3.4M row index spanning 26 countries, deduplicated across sources, with a pay estimate only where the employer stated nothing, semantic CV matching against the full corpus, and a seven-stage tracker for everything applied to.",
    ...
    approach: [
      ...
      "Matching is three separate paths, deliberately not one. Keyword ATS extraction parses the CV in the browser and uploads nothing. Semantic similarity scores a saved 384-dimension multilingual CV embedding against job embeddings in pgvector, over the full corpus, and reports a posting that names no recognised skill as too thin to score rather than as a confident 0%. Claude scores individual jobs and drafts cover letters and interview prep, under a per-user daily quota, with prompt-injection defences on every call and fail-open behaviour that is documented rather than incidental. The second and third need an account and a stored CV, which is Fernet-encrypted at rest, and the backend refuses to start in production without the key.",
    ],
    decisions: [
      {
        title: "Drop a pay figure rather than reinterpret it",
        body: "Zero is dropped: it means the provider had nothing, never that the job is unpaid. A figure whose annual equivalent is implausible is dropped too, not converted. I know GBP 450 is not an annual salary, but I do not know whether it is a day rate, an hourly rate or a typo, and guessing daily would replace a wrong fact with an invented one. Pay renders in the period the employer quoted, or it does not render.",
      },
      {
        title: "Exclude one provider's predicted salaries",
        body: "Adzuna flags when a figure is its own market estimate rather than the employer's. Measured on 2026-08-18, 49 of 50 US data-scientist rows that carried pay were predicted. No plausibility check catches that, because a model's output is plausible by construction, so those figures were rendering as quotes under a promise of employer-stated pay. They are out of aggregation entirely now, and styled distinctly where they still appear.",
      },
      {
        title: "Train the pay model, then leave it on the shelf",
        body: "The shipped estimate is a lookup over measured cells, so the obvious upgrade is a regression. I trained one against that lookup as the baseline, split by employer group so the same poster could not sit on both sides of the split: MAE 27,420 down to 24,001, MdAPE 18.9% down to 16.4%, reported per country and seniority rather than as one headline. It is not deployed. A model output is plausible by construction, which is the exact failure I already excluded one provider for, and a 16% median error is not good enough to start speaking where the employer did not.",
      },
      {
        title: "Only positive evidence marks a listing dead",
        body: "Nothing revalidated a stored posting until August. On the newest 40 rows per site, 12.1% of what was being served was already gone, and on two boards it was over half. So a sweep reprobes stored URLs, but a 403 or a timeout writes nothing at all: 170 of 593 probes were refusals, and treating a refusal as an answer would have buried 168,238 live rows in a single pass. Going faster makes it worse rather than better. At concurrency 10, 126 of 200 probes came back unknown.",
      },
      {
        title: "Dedupe at write time and on every read",
      ...
      {
        title: "Ranking in a Postgres function, not the query builder",
        body: "The shape this replaced shipped 27.4% of results with none of the user's search words in the job title. The fix is a full-text index weighting the title above the description, and those weights are only read by ts_rank_cd, so the weighting and the ranking function are one change in two files and neither does anything alone. There is no paid placement in the ranking: sponsorship came out of the function in a migration.",
      },
      {
        title: "Keyword scoring in the browser, everything else opt-in",
        body: "Server-side scoring would cache better and extract more, but it makes a stored CV the default rather than a choice. Keeping the keyword path local makes the data minimisation structural rather than a policy promise, and the two paths that genuinely need a stored CV ask for one explicitly instead of quietly assuming it.",
      },
      {
        title: "SSE over WebSocket",
        body: "Search progress is a one-way push from server to client. SSE is a single HTTP request, reconnects on its own, and skips the upgrade dance. A WebSocket would be over-specified for a stream that never carries anything upstream.",
      },
    ],
    results: [
      "A 3.4M row index across 26 countries, over a taxonomy of 1,173 keywords in 42 groups. On the 2026-08-17 census, 82.7% of the corpus had been posted within the previous 30 days. 29 sources are integrated: 23 job boards and 6 ATS platforms ingested directly.",
      "The source count has grown from six at launch to twenty-nine with no architectural change. The parallel, rate-limited fan-out absorbed every new one, which is the strongest evidence that the ingest shape was right.",
      ...
      "Operationally: 23 RLS-locked tables over 43 migrations, 1,950 automated tests behind 11 CI build gates, GDPR export and deletion, PKCE OAuth for Google, LinkedIn and GitHub, header-only admin auth with an audit log that escalates to Sentry if its own write fails, and 200 generated category pages of which 2.0% ship noindexed against a build gate set at 15%.",
    ],
    reflections: [
      "The claim I most want to correct is one I made myself. This page used to say 2M postings, 60+ countries, and first results in about 1.4 seconds, and not one of the three had been measured. The index passed 2M for real on a sweep with a date on it, which is the least comfortable version of being right: the number was a guess that happened to land near the truth, and a guess that lands is still a guess. The country figure was worse and was withdrawn outright - it counted distinct search-location strings, so it was counting query regions and multi-counting any country I had searched under several city names. The 26 above is a different measurement: countries with rows actually in the index. Nothing has timed the first-result path since June, so there is still no timing figure on this page.",
      "Two things the numbers above deliberately do not say. Coverage is uneven: on the last census five sources accounted for 77.8% of everything and six boards had contributed no rows at all, so the honest verb is that a search queries up to 29 sources, not that the index draws on 29 evenly. And liveness had been probed on 0.32% of the corpus. The filter ships; the coverage does not exist yet, and claiming otherwise would be exactly the failure this project is built to avoid.",
